...
const CustomerTypeBadge = ({ type }) => {
  const colorMap = { B2B: 'primary', B2C: 'success', Dealer: 'warning', Wholesale: 'info', Retail: 'secondary' };
  return (
    <Badge bg={colorMap[type] || 'secondary'} className="fw-normal rounded-pill px-2">
      {type}
    </Badge>
  );
};
...
const CustomerMasterPage = () => {
  ...
  const [addForm,      setAddForm]      = useState({ ...EMPTY_CUSTOMER, Addresses: [{ ...EMPTY_ADDRESS }] });
  ...
  // Load states when country changes in add form
  useEffect(() => {
    const cid = addForm.Addresses?.[0]?.CountryId;
    if (!cid) { setStates([]); return; }
    apiClient.get(`/api/v1/inventory/states?country_id=${cid}`).then(res => {
      setStates(res.data || res || []);
    }).catch(() => setStates([]));
  }, [addForm.Addresses?.[0]?.CountryId]);

  // Load districts when state changes
  useEffect(() => {
    const sid = addForm.Addresses?.[0]?.StateId;
    if (!sid) { setDistricts([]); return; }
    apiClient.get(`/api/v1/inventory/districts?state_id=${sid}`).then(res => {
      setDistricts(res.data || res || []);
    }).catch(() => setDistricts([]));
  }, [addForm.Addresses?.[0]?.StateId]);

  // Load cities when state changes
  useEffect(() => {
    const sid = addForm.Addresses?.[0]?.StateId;
    if (!sid) { setCities([]); return; }
    apiClient.get(`/api/v1/inventory/cities?state_id=${sid}`).then(res => {
  ...
  const updateAddressField = (field, value) => {
    setAddForm(f => ({
      ...f,
      Addresses: f.Addresses.map((addr, i) => i === 0 ? { ...addr, [field]: value } : addr)
    }));
  };

  const handleAdd = async () => {
    if (!addForm.CustomerName.trim() || !addForm.PAN.trim()) {
      showAlert('Customer Name and PAN are required', 'danger'); return;
    }
    setAddSaving(true);
    try {
      await apiClient.post('/api/v1/inventory/customers', addForm);
      setShowAddModal(false);
      setAddForm({ ...EMPTY_CUSTOMER, Addresses: [{ ...EMPTY_ADDRESS }] });
      setAddTab('basic');
      fetchCustomers();
  ...
  const customerTypes = ['B2B', 'B2C', 'Dealer', 'Wholesale', 'Retail'];
  ...
  return (
    <div className="container-fluid p-0">
      ...
      {/* Alert */}
      {alert && (
        <div className={`alert alert-${alert.type} alert-dismissible position-fixed top-0 end-0 m-3 shadow`} style={{ zIndex: 9999, minWidth: 280 }}>
          {alert.msg}
        </div>
      )}
          ...
          <div className="table-responsive">
            ...
            <div className="d-block d-md-none">
              {customers.map(c => {
                const editing = editingId === c.id;
                return (
                  <div key={c.id} className={`p-3 border-bottom ${editing ? 'bg-warning bg-opacity-10' : ''}`} onDoubleClick={() => !editing && handleDoubleClick(c)}>
      ...
      {/* ── Add Customer Modal ── */}
      <Modal show={showAddModal} onHide={() => { setShowAddModal(false); setAddTab('basic'); }} centered size="lg">
        ...
        <Modal.Body>
          <Tab.Container activeKey={addTab} onSelect={k => setAddTab(k)}>
            <Nav variant="tabs" className="mb-3 border-bottom">
              ...
              <Nav.Item>
                <Nav.Link eventKey="address" className="d-flex align-items-center gap-1">
                  <MapPin size={15} /> Address
                </Nav.Link>
              </Nav.Item>
              <Nav.Item>
                <Nav.Link eventKey="bank" className="d-flex align-items-center gap-1">
                  <CreditCard size={15} /> Bank / Contact
                </Nav.Link>
              </Nav.Item>
            </Nav>
            ...
            <Tab.Content>
              ...
              <Tab.Pane eventKey="address">
                <Row className="g-3">
                  <Col xs={12} md={6}>
                    <Form.Group>
                      <Form.Label className="fw-medium small">Address Type</Form.Label>
                      <Form.Select value={addForm.Addresses[0].AddressType} onChange={e => updateAddressField('AddressType', e.target.value)}>
                  ...
                  <Col xs={12} md={6}>
                    <Form.Group>
                      <Form.Label className="fw-medium small">Pincode</Form.Label>
                      <Form.Control placeholder="e.g. 400001" maxLength={6} value={addForm.Addresses[0].Pincode} onChange={e => updateAddressField('Pincode', e.target.value)} />
                    </Form.Group>
                  </Col>
                  <Col xs={12}>
                    <Form.Group>
                      <Form.Label className="fw-medium small">Address Line 1</Form.Label>
                      <Form.Control placeholder="Street, Building No." value={addForm.Addresses[0].AddressLine1} onChange={e => updateAddressField('AddressLine1', e.target.value)} />
                    </Form.Group>
                  </Col>
                  <Col xs={12}>
                    <Form.Group>
                      <Form.Label className="fw-medium small">Address Line 2</Form.Label>
                      <Form.Control placeholder="Area, Landmark (optional)" value={addForm.Addresses[0].AddressLine2} onChange={e => updateAddressField('AddressLine2', e.target.value)} />
                    </Form.Group>
                  </Col>
                  <Col xs={12} md={6}>
                    <Form.Group>
                      <Form.Label className="fw-medium small">Country</Form.Label>
                      <Form.Select value={addForm.Addresses[0].CountryId} onChange={e => updateAddressField('CountryId', e.target.value)}>
                  ...
                  <Col xs={12} md={6}>
                    <Form.Group>
                      <Form.Label className="fw-medium small">State</Form.Label>
                      <Form.Select value={addForm.Addresses[0].StateId} onChange={e => updateAddressField('StateId', e.target.value)} disabled={!addForm.Addresses[0].CountryId}>
                  ...
                  <Col xs={12} md={6}>
                    <Form.Group>
                      <Form.Label className="fw-medium small">District</Form.Label>
                      <Form.Select value={addForm.Addresses[0].DistrictId} onChange={e => updateAddressField('DistrictId', e.target.value)} disabled={!addForm.Addresses[0].StateId}>
                  ...
                  <Col xs={12} md={6}>
                    <Form.Group>
                      <Form.Label className="fw-medium small">City</Form.Label>
                      <Form.Select value={addForm.Addresses[0].CityId} onChange={e => updateAddressField('CityId', e.target.value)} disabled={!addForm.Addresses[0].StateId}>
                  ...
                  <Col xs={12} md={6}>
                    <Form.Check
                      ...
                      checked={addForm.Addresses[0].IsDefault === 1}
                      onChange={e => updateAddressField('IsDefault', e.target.checked ? 1 : 0)}
                      className="mt-3"
                    />
                  </Col>
                </Row>
              ...
              <Tab.Pane eventKey="bank">
                <Row className="g-3">
                  <Col xs={12} md={6}>
                    <Form.Group>
                      <Form.Label className="fw-medium small">Contact Person</Form.Label>
                      <Form.Control placeholder="Contact name at this address" value={addForm.Addresses[0].ContactPerson} onChange={e => updateAddressField('ContactPerson', e.target.value)} />
                    </Form.Group>
                  </Col>
                  <Col xs={12} md={3}>
                    <Form.Group>
                      <Form.Label className="fw-medium small">Mobile No</Form.Label>
                      <Form.Control placeholder="9XXXXXXXXX" value={addForm.Addresses[0].MobileNo} onChange={e => updateAddressField('MobileNo', e.target.value)} />
                    </Form.Group>
                  </Col>
                  <Col xs={12} md={3}>
                    <Form.Group>
                      <Form.Label className="fw-medium small">WhatsApp No</Form.Label>
                      <Form.Control placeholder="9XXXXXXXXX" value={addForm.Addresses[0].WhatsAppNo} onChange={e => updateAddressField('WhatsAppNo', e.target.value)} />
                    </Form.Group>
                  </Col>
                  <Col xs={12} md={6}>
                    <Form.Group>
                      <Form.Label className="fw-medium small">Bank Name</Form.Label>
                      <Form.Control placeholder="e.g. HDFC Bank" value={addForm.Addresses[0].BankName} onChange={e => updateAddressField('BankName', e.target.value)} />
                    </Form.Group>
                  </Col>
                  <Col xs={12} md={6}>
                    <Form.Group>
                      <Form.Label className="fw-medium small">Account Number</Form.Label>
                      <Form.Control placeholder="Account number" value={addForm.Addresses[0].AccountNumber} onChange={e => updateAddressField('AccountNumber', e.target.value)} />
                    </Form.Group>
                  </Col>
                  <Col xs={12} md={6}>
                    <Form.Group>
                      <Form.Label className="fw-medium small">IFSC Code</Form.Label>
                      <Form.Control placeholder="e.g. HDFC0001234" maxLength={11} value={addForm.Addresses[0].IFSCCode} onChange={e => updateAddressField('IFSCCode', e.target.value.toUpperCase())} />
                    </Form.Group>
                  </Col>
                  <Col xs={12} md={6}>
                    <Form.Group>
                      <Form.Label className="fw-medium small">Branch Name</Form.Label>
                      <Form.Control placeholder="e.g. Andheri West" value={addForm.Addresses[0].BranchName} onChange={e => updateAddressField('BranchName', e.target.value)} />
                    </Form.Group>
                  </Col>
                </Row>
              </Tab.Pane>
        ...
        <Modal.Footer className="border-0 pt-0">
          <Button variant="light" className="rounded-pill px-4" onClick={() => { setShowAddModal(false); setAddTab('basic'); }}>Cancel</Button>
          <Button
            style={{ background: 'linear-gradient(135deg, #8b5cf6, #6366f1)', border: 'none' }}
